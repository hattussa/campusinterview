...
// Handle visibility change events and restart quiz immediately on tab switch or minimize
document.addEventListener('visibilitychange', () => {
  if (document.hidden) {
    ...
    const correctPassword = "hci123@#$";

    // Define the start and end date/time
    const startDate = new Date("2025-03-03T08:00:00"); // Start date/time
    const endDate = new Date("2025-03-04T15:00:00"); // End date/time
...
let quizData = [];
let timer;
let timeLeft = 4500; // 1 hour 15 minutes in seconds
...
function submitToGoogleSheet(data) {
  ...
  fetch(urlWithParams, {
    ...
    .catch(error => {
      ...
      alert(error.message || 'An error occurred. Please check the console for details.');
    });
}
